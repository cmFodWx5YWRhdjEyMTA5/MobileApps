  ...
  _sendCoins()
  {
    ...
    if(coin > 0)
    {
      if(this.state.coin==='')
      {
        ...
        createTransaction.push({ coins:this.state.coin ,createdAt:dt,frinedId:this.state.frndId, planType:'GIFT2FRIEND', trxType:'DEBIT',userId:this.state.userId}).then(()=>{
          ...
          currentUserRef.update({coinBalance:userCoins}).then(()=>{
              //Plus Coins To Friend Database
                  if(this.state.isNullCoinKey===true)
                  {
                      var frndData = firebase.database().ref('User').child(this.state.frndId);
                      frndData.update({coinBalance:this.state.coin})
                  }else{
                    var coinAdd = parseInt(this.state.frndCoin) + parseInt(this.state.coin)
                    var frndData = firebase.database().ref('User').child(this.state.frndId);
                    frndData.update({coinBalance:coinAdd}).then(()=>{
